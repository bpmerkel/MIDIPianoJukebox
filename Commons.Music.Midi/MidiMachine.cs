...
public class MidiMachineChannel
{
    ...
    public short RpnTarget => (short)((Controls[MidiCC.RpnMsb] << 7) + Controls[MidiCC.RpnLsb]);
    ...
    public void ProcessDte(byte value, bool msb)
    {
        ...
        var cur = arr[dte_target];
        arr[dte_target] = msb
            ? (short)(cur & 0x007F + ((value & 0x7F) << 7))
            : (short)(cur & 0x3FF0 + (value & 0x7F));
    }
